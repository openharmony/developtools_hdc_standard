...
step2： 先检查pc(host)端设备管理器设备信息是否正常（设备是否正常加载显示、驱动、id是否正常）；
...
3. windows版本工具无法执行报错：1）确认git lfs正常并使用git clone下载本工具仓预制二进制文件；2）检查文件大小和版本； 3）友情提示：本工具是控制台命令行下执行即可，不需要安装，不要尝试双击方式运行；